...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  signInWithGitHub(SignInWithGitHubEvent event, Emitter<AuthState> emit) async{
    ...
    if (gitHubSignInResult.status == GitHubSignInResultStatus.ok) {
      ...
      await SharedPreferencesService.saveString(user.additionalUserInfo?.username ?? '',Constants.username);
      await SharedPreferencesService.saveString(user.credential?.accessToken ?? '',Constants.token);
      await SharedPreferencesService.saveString(user.additionalUserInfo?.profile?['login'] ?? '',Constants.login);
      await SharedPreferencesService.saveString(user.additionalUserInfo?.profile?['avatar_url'] ?? '',Constants.profile);
      emit(AuthSignedIn());
    } else {
